...
/*
 * This is the data that's used to test this program. It forms a tree that looks like this:
 *
 *               64
 *              /  \
 *             /    \
...
/*
 * This array defines some range sums to compute within the tree defined above. Each range sum is specified as a triple: {lower, upper, sum}, where `lower` and `upper` are respectively the lower and upper bounds of the range and sum` is the true sum of keys in that range.
 */
...
/*
 * This is a helper function that's used to compare integers when sorting with qsort().
 */
...
int main(int argc, char** argv) {
  /*
   * Create a new BST and insert the testing data into it. The testing data is inserted in such a way that the value is equal to the address of the corresponding key (which comes from the TEST_DATA array above). In other words, for any given key, we'll be able to verify that the tree contains the correct value by comparing key == *value.
   */
  printf("== Creating BST...\n");
  ...
  /*
   * Test that we can't successfully find any values we know shouldn't be in the BST. To do this, create a sorted copy of the test data that will allow us to more easily determine which values shouldn't be in the BST.
   */
  ...
  /*
   * Test BST path sum function, checking several values that the tree should and have as path sums.
   */
  ...
  for (int i = 0; i < NUM_RANGE_SUMS; i++) {
    ...
    printf("  -- bst_range_sum(%d, %d): %d (expected %d)\n", lower, upper, bst_range_sum(bst, lower, upper), sum);
  }


  /*
   * Test removing keys from the BST. After removing each key, make sure it's no longer present in the BST.
   */
  ...
  /*
   * Test that all of the values we know should still be in the BST are indeed there.
   */
